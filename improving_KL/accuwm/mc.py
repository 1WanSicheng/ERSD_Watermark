...
def gen_mc(
    ...
    if fully_coupled and not got_eos:
        ...
        if logprobs_tail.shape[-1] != logprobs.shape[-1]:
            logprobs_tail = logprobs_tail[..., : logprobs.shape[-1]]
    ...
    # fix past_key_values
    past_key_values = truncate_cache(
        output.past_key_values, input_ids.shape[1] + output_ids.shape[1] - 1
    )
...
def mc_sample_generator_full(
    ...
    **kwargs
    ...
    model.eval()
    ref_model.eval()
    if ref_past_key_values is not None and not isinstance(
        ref_past_key_values, (list, tuple, DynamicCache)
    ):
        ref_past_key_values = None
    while True:
