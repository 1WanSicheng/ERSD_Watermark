...
def partition_local_mpfr_tokens_from_logprobs(
    target_logprobs: FloatTensor,
    proposal_logprobs: FloatTensor,
    source: SharedPFRSource,
    num_samples: int,
    *,
    partition_top_m: int = 256,
    partition_tail_bins: int = 32,
    low_ratio_singleton_threshold: float = 1e-2,
    max_proposals: int = 100_000,
    allow_incomplete: bool = False,
    return_result: bool = False,
    **_unused_kwargs,
    ...
    """
    Partition-local exact finite MPFR.
    ...
    The vocabulary is partitioned into cells. For each cell C,
        q_C = Q(C),
        w_C = min_{y in C} Q(y)/P(y).

    The lower-bound process L_{C,k} = w_C tau_{C,k} has rate q_C / w_C.
    We simulate the superposition of these lower-bound processes directly.
    The stopping rule is exact:
        current B-th smallest true score <= next lower-bound event.

    This preserves exactness of the first B mapped-PFR samples, while avoiding
    the global worst-case w_min stopping rule.
    """
    device = target_logprobs.device
    out_dtype = target_logprobs.dtype

    if num_samples <= 0:
        empty_tokens = torch.empty((0,), device=device, dtype=torch.long)
        if return_result:
            return FiniteMPFRResult(
                tokens=empty_tokens,
                scores=torch.empty((0,), device=device, dtype=out_dtype),
                proposal_indices=torch.empty((0,), device=device, dtype=torch.long),
                num_proposals=0,
                w_lower=float("nan"),
                terminated=True,
            )
    ...
    if max_proposals < num_samples:
        raise ValueError("max_proposals must be at least num_samples")
    if partition_top_m < 0:
        raise ValueError("partition_top_m must be nonnegative")
    if partition_tail_bins < 0:
        raise ValueError("partition_tail_bins must be nonnegative")
    if low_ratio_singleton_threshold <= 0:
        raise ValueError("low_ratio_singleton_threshold must be positive")

    target_logprobs = _as_1d_logprobs(target_logprobs, "target_logprobs").float()
    proposal_logprobs = _as_1d_logprobs(proposal_logprobs, "proposal_logprobs").float()
    if target_logprobs.shape != proposal_logprobs.shape:
        raise ValueError("target_logprobs and proposal_logprobs must have the same shape")
    ...
    proposal_positive = torch.isfinite(proposal_logprobs)
    if bool((target_positive & ~proposal_positive).any().item()):
        raise ValueError("partition-local MPFR requires P to be absolutely continuous with respect to Q")

    # Work on CPU NumPy arrays for partition bookkeeping and alias sampling.
    # This is a per-context finite-vocabulary algorithm.
    target_probs_t = torch.zeros_like(target_logprobs)
    proposal_probs_t = torch.zeros_like(proposal_logprobs)

    target_probs_t[target_positive] = torch.exp(target_logprobs[target_positive])
    proposal_probs_t[proposal_positive] = torch.exp(proposal_logprobs[proposal_positive])

    target_mass = float(target_probs_t.sum().item())
    proposal_mass = float(proposal_probs_t.sum().item())
    if target_mass <= 0:
        raise ValueError("target distribution has empty support")
    if proposal_mass <= 0:
        raise ValueError("proposal distribution has empty support")

    target_probs_t = target_probs_t / target_mass
    proposal_probs_t = proposal_probs_t / proposal_mass

    P = target_probs_t.detach().cpu().numpy().astype(np.float64)
    Q = proposal_probs_t.detach().cpu().numpy().astype(np.float64)
    V = P.shape[0]
    ...
    ratio = np.full(V, np.inf, dtype=np.float64)
    ratio[active] = Q[active] / P[active]
    log_ratio = np.full(V, np.inf, dtype=np.float64)
    log_ratio[active] = np.log(Q[active]) - np.log(P[active])

    global_w_lower = float(np.min(ratio[active]))
    if not math.isfinite(global_w_lower) or global_w_lower <= 0:
        raise ValueError("invalid global lower bound")

    # Build partition:
    # 1. singleton top tokens under P or Q;
    # 2. singleton very low-ratio tokens;
    # 3. remaining active tokens binned by log(Q/P) with roughly equal Q-mass.
    top_m = min(int(partition_top_m), active_idx.size)

    if top_m > 0:
        top_p = active_idx[np.argsort(-P[active_idx])[:top_m]]
        top_q = active_idx[np.argsort(-Q[active_idx])[:top_m]]
    else:
        top_p = np.empty((0,), dtype=np.int64)
        top_q = np.empty((0,), dtype=np.int64)

    low_ratio = active_idx[ratio[active_idx] <= low_ratio_singleton_threshold]
    singleton_tokens = np.unique(np.concatenate([top_p, top_q, low_ratio])).astype(np.int64)

    is_singleton = np.zeros(V, dtype=bool)
    is_singleton[singleton_tokens] = True

    cells: list[np.ndarray] = []
    for y in singleton_tokens:
        cells.append(np.array([int(y)], dtype=np.int64))

    tail = active_idx[~is_singleton[active_idx]]
    if tail.size > 0:
        if partition_tail_bins <= 1:
            cells.append(tail.astype(np.int64))
        else:
            tail_sorted = tail[np.argsort(log_ratio[tail])]
            total_tail_mass = float(np.sum(Q[tail_sorted]))
            target_bin_mass = total_tail_mass / float(partition_tail_bins)

            current: list[int] = []
            current_mass = 0.0
            made_bins = 0

            for y in tail_sorted:
                current.append(int(y))
                current_mass += float(Q[y])

                if current_mass >= target_bin_mass and made_bins < partition_tail_bins - 1:
                    cells.append(np.array(current, dtype=np.int64))
                    current = []
                    current_mass = 0.0
                    made_bins += 1
    ...
    class _AliasSampler:
        def __init__(self, probs_np: np.ndarray):
            probs_np = np.asarray(probs_np, dtype=np.float64)
            total = float(np.sum(probs_np))
            if total <= 0:
                raise ValueError("cannot build alias sampler for zero-mass distribution")
            probs_np = probs_np / total

            self.n = len(probs_np)
            scaled = probs_np * self.n
            self.cutoff = np.zeros(self.n, dtype=np.float64)
            self.alias = np.arange(self.n, dtype=np.int64)

            small = list(np.where(scaled < 1.0)[0])
            large = list(np.where(scaled >= 1.0)[0])

            while small and large:
                s = small.pop()
                l = large.pop()

                self.cutoff[s] = scaled[s]
                self.alias[s] = l
        ...
        def sample_one(self, rng: np.random.Generator) -> int:
            ...
            if rng.random() < self.cutoff[k]:
                return k
            return int(self.alias[k])

    clean_cells: list[np.ndarray] = []
    q_mass_list: list[float] = []
    w_cell_list: list[float] = []
    alias_cells: list[_AliasSampler] = []

    for cell in cells:
        q_mass = float(np.sum(Q[cell]))
        if q_mass <= 0:
            continue

        w_cell = float(np.min(ratio[cell]))
        if not math.isfinite(w_cell) or w_cell <= 0:
            raise ValueError("partition cell has invalid local lower bound")

        clean_cells.append(cell)
        q_mass_list.append(q_mass)
        w_cell_list.append(w_cell)
        alias_cells.append(_AliasSampler(Q[cell] / q_mass))

    if not clean_cells:
        raise ValueError("partition has no nonempty active cells")

    q_mass_arr = np.array(q_mass_list, dtype=np.float64)
    w_cell_arr = np.array(w_cell_list, dtype=np.float64)

    # Lower-bound rate for cell C:
    #     lambda_C = Q(C) / w_C.
    lb_rate = q_mass_arr / w_cell_arr
    lb_rate_sum = float(np.sum(lb_rate))
    if not math.isfinite(lb_rate_sum) or lb_rate_sum <= 0:
        raise ValueError("invalid lower-bound rate sum")

    alias_lb_cells = _AliasSampler(lb_rate)

    rng = source.numpy_rng()

    # Event-driven lower-bound simulation.
    next_lower_bound = float(rng.exponential(scale=1.0) / lb_rate_sum)
    best_heap: list[tuple[float, int, int]] = []
    num_processed = 0
    terminated = False

    while num_processed < max_proposals:
        if len(best_heap) == num_samples:
            current_bth_score = -best_heap[0][0]
            if current_bth_score <= next_lower_bound:
                terminated = True
                break

        lower_time = next_lower_bound
        ell = alias_lb_cells.sample_one(rng)
        cell = clean_cells[ell]

        local_pos = alias_cells[ell].sample_one(rng)
        token = int(cell[local_pos])

        # True score:
        #   S = lower_time * (Q(token)/P(token)) / w_cell.
        log_multiplier = log_ratio[token] - math.log(w_cell_arr[ell])
        if log_multiplier > 700:
            score = float("inf")
        else:
            score = float(lower_time * math.exp(log_multiplier))

        event_index = num_processed + 1
        if len(best_heap) < num_samples:
            heapq.heappush(best_heap, (-score, event_index, token))
        else:
            current_worst = -best_heap[0][0]
            if score < current_worst:
                heapq.heapreplace(best_heap, (-score, event_index, token))
    ...
    if not terminated and not allow_incomplete:
        raise RuntimeError(
            "partition-local MPFR did not terminate within max_proposals; "
            "increase max_proposals or refine the partition"
        )

    selected = [(-neg_s, event_idx, token) for neg_s, event_idx, token in best_heap]
    selected.sort(key=lambda triple: triple[0])

    tokens_np = np.array([token for _, _, token in selected], dtype=np.int64)
    scores_np = np.array([score for score, _, _ in selected], dtype=np.float32)
    indices_np = np.array([event_idx for _, event_idx, _ in selected], dtype=np.int64)

    tokens = torch.from_numpy(tokens_np).to(device=device, dtype=torch.long)
    if not return_result:
        return tokens

    scores = torch.from_numpy(scores_np).to(device=device, dtype=out_dtype)
    proposal_indices = torch.from_numpy(indices_np).to(device=device, dtype=torch.long)

    return FiniteMPFRResult(
        tokens=tokens,
        scores=scores,
        proposal_indices=proposal_indices,
        num_proposals=num_processed,
        w_lower=global_w_lower,
        terminated=terminated,
    )
...
def finite_mpfr_tokens_from_proposal(
    ...
    **kwargs,
    ...
    if proposal == "partition_local_uniform":
        target_logprobs = _as_1d_logprobs(target_logprobs, "target_logprobs")
        target_probs = target_logprobs.float().exp()
        target_mass = target_probs.sum()
        if float(target_mass.item()) <= 0:
        ...
        return partition_local_mpfr_tokens_from_logprobs(
            _safe_log(target_probs),
            proposal_logprobs,
            source=source,
            num_samples=num_samples,
    ...
    if proposal == "partition_local_model":
        target_logprobs = _as_1d_logprobs(target_logprobs, "target_logprobs")
        return partition_local_mpfr_tokens_from_logprobs(
            target_logprobs,
            target_logprobs,
