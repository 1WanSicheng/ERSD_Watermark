...
def run_prompt(
    ...
    for step_output_ids, _step_output_logprobs, meta in gen:
        ...
        draft_context_counts.append(int(meta["draft_context_count"]))
        target_context_counts.append(int(meta["target_context_count"]))
        active_counts = meta.get("active_counts", [])
        active_count_means.append(float(np.mean(active_counts)) if active_counts else 0.0)
        generated += block_len
    ...
    return {
        ...
        "draft_context_count_mean": (
            float(np.mean(draft_context_counts)) if draft_context_counts else 0.0
        ),
        "target_context_count_mean": (
            float(np.mean(target_context_counts)) if target_context_counts else 0.0
        ),
        "active_count_mean": float(np.mean(active_count_means)) if active_count_means else 0.0,
...
def aggregate(rows):
    ...
    return {
        ...
        "draft_context_count_mean": float(
            np.mean([row["draft_context_count_mean"] for row in rows])
        ),
        "target_context_count_mean": float(
            np.mean([row["target_context_count_mean"] for row in rows])
        ),
        "active_count_mean": float(np.mean([row["active_count_mean"] for row in rows])),
