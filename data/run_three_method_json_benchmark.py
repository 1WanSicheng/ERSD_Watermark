...
# Ashish / list-level implementation via Transformers-compatible wrapper.
from list_level_baseline_local_nocache import list_level_identical_generator  # type: ignore
from list_level_invariant_local_nocache import list_level_invariant_generator  # type: ignore
...
def run_ashish_strong(
    *,
    ...
    """Run Ashish/list-level identical baseline via a Transformers-compatible wrapper.

    The original SpeculativeDecoding/strategy.py uses old tuple-style cache
    indexing, while recent Transformers/Qwen2 returns DynamicCache.  The wrapper
    list_level_identical_generator implements the same strong/list-level
    identical scheme and returns explicit metadata for AATPS/BE/forward calls.
    """
    L = int(config.get("max_draft_len", 4))
    B = int(config.get("max_num_drafts", 2))
    process_logits_kwargs = {
        "temperature": normalize_temperature(config.get("temperature", 1.0), B),
        "top_k": int(config.get("top_k", 50)),
        "top_p": float(config.get("top_p", 1.0)),
    }
    ...
    baseline_fn = list_level_invariant_generator if config.get('method') == 'ashish_invariant' else list_level_identical_generator
    ...
    for yielded in gen:
        ...
        attempted_total += int(meta.get("attempted_draft_tokens", L))
        target_calls += int(meta.get("target_forward_calls", meta.get("target_context_count", 1)))
        draft_calls += int(meta.get("draft_forward_calls", meta.get("draft_tree_size", L)))
        ...
        for dataset_name in cfg["datasets"]:
            ...
            for config in cfg["configurations"]:
                ...
                for i in tqdm(range(min(warmup, len(dataset))), desc=f"Warmup {desc}"):
                    ...
                    if method in {"ashish_strong", "ashish_invariant"}:
