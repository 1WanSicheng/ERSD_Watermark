...
DISPLAY = {
    ...
    "pfr": "PFR (ours)",
    "mpfr_torchgen_cached": "MPFR (ours)",
}
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--label", default="Qwen+CNN", help="Prefix used in the top-panel title (e.g. 'Qwen+CNN').")
...
def is_multi_draft(obj: dict[str, Any]) -> bool:
    cfg = obj.get("config", {})
    if cfg.get("experiment") == "multi_draft":
        return True
    # Heuristic: multi-draft files declare num_drafts in config.
    if "num_drafts" in cfg:
        return True
    return False
...
def make_top_panel(ax, summary_all: pd.DataFrame, mmap: dict[int, str], label: str = "Qwen+CNN") -> None:
    scatter_points(ax, summary_all, xcol="AATPS_mean", ycol="ANLPPT_U_mean", mmap=mmap, include_b_labels=True)
    ax.set_title(f"{label}: AATPS--ANLPPT-U tradeoff with MPFR")
    ax.set_xlabel("AATPS")
...
def make_figure(summary_single: pd.DataFrame, mpfr: pd.DataFrame, out: Path, *, dpi: int, figwidth: float, figheight: float, label: str = "Qwen+CNN") -> None:
    ...
    make_top_panel(ax_top, summary_all, mmap, label=label)
...
def main() -> None:
    ...
    make_figure(summary_single, mpfr, Path(args.out), dpi=args.dpi, figwidth=args.figwidth, figheight=args.figheight, label=args.label)
